...
// Ratiorma独自の相対評価ロジック（ボーダーラインを現実に最適化＆コンサル分析を超強化）
const getDiagnosticResult = (viewCount, cvrStr) => {
  ...
  if (viewCount >= 1000000) {
    // メガヒット領域（100万再生超）
    if (cvr >= 0.6) return { 
      icon: '👑', rank: 'ユニコーン企業', sub: 'Unicorn / M-1決勝レベル', label: 'メガヒット領域（100万再生超）', 
      text: `圧倒的なマスへの到達（${(viewCount/10000).toFixed(0)}万再生）を達成しながら、CVR ${cvr}%という驚異的なエンゲージメントを維持している稀有な事例である。一般層への「ウケ」と、お笑いフリークを唸らせる「ネタの強度」が完全に同居しており、M-1決勝の舞台において審査員と観客の双方を掌握できる完全無欠のポテンシャルを誇る。もはや単なるネタ動画の枠を超え、強固なブランドとして市場に君臨するメガ・ユニコーン銘柄である。` 
    };
    if (cvr >= 0.35) return { 
      icon: '💎', rank: '優良事業', sub: 'Cash Cow / M-1・準々決勝～準決勝レベル', label: 'メガヒット領域（100万再生超）', 
      text: `${(viewCount/10000).toFixed(0)}万再生という巨大な分母に対し、CVR ${cvr}%は非常に安定的かつ優秀なスコアである。一過性のバズ消費に終わらず、視聴者が確実にネタの本質的価値を評価し、強固な支持基盤（キャッシュカウ）を形成している。賞レースの過酷な予選を突破する地力はすでに証明済みであり、決勝進出に向けては「この組ならではの爆発的フック」を一点突破で研ぎ澄ますフェーズに入っている。` 
    };
    if (cvr >= 0.15) return { 
      icon: '🌱', rank: '損益分岐点', sub: 'Break-Even Point / M-1・2回戦・3回戦レベル', label: 'メガヒット領域（100万再生超）', 
      text: `メガヒット領域に到達し、マジョリティ層への認知拡大には成功しているものの、能動的な評価（高評価率）は市場の平均値に収束している。YouTubeのアルゴリズムによる露出恩恵を多分に受けている可能性が高く、視聴者が「コアなファン」へ転換する手前の段階（損益分岐点）に留まっている。今後は、広く見られることよりも「深く刺さる」ための、ネタの根幹となるUSP（独自の強み）の再定義が求められる。` 
    };
    return { 
      icon: '📉', rank: '不良債権', sub: 'Non-Performing Loan / M-1・1回戦敗退レベル', label: 'メガヒット領域（100万再生超）', 
      text: `莫大なインプレッションを獲得しているにも関わらず、CVR ${cvr}%という数値は、ターゲット層との深刻なミスマッチを示唆している。サムネイルやタイトルによる一時的な誘引には成功しているが、コンテンツの実態が視聴者の期待値に応えられていない。現状のままでは賞レースでの評価獲得は極めて困難であり、小手先の修正ではなく、漫才のシステムそのものに対する抜本的な構造改革（リストラクチャリング）が急務である。` 
    };
  } 
  ...
  if (viewCount >= 100000) {
    // ミドルヒット領域（10万再生超）
    if (cvr >= 1.5) return { 
      icon: '👑', rank: 'ユニコーン企業', sub: 'Unicorn / M-1決勝レベル', label: 'ミドルヒット領域（10万再生超）', 
      text: `10万再生を超えるリーチを持ちながら、CVR ${cvr}%という外れ値（アウトライアー）を叩き出している。これは特定のターゲット層に極めて深く、そして鋭く刺さっている証左であり、熱狂的なコミュニティが形成されつつある。この「異常な熱量」を保ったままマスへと露出を拡大できれば、一気に業界の勢力図を塗り替える大化け銘柄（ユニコーン）となる確実なポテンシャルを秘めている。` 
    };
    if (cvr >= 0.8) return { 
      icon: '💎', rank: '優良事業', sub: 'Cash Cow / M-1・準々決勝～準決勝レベル', label: 'ミドルヒット領域（10万再生超）', 
      text: `再生数とエンゲージメントのバランスが美しく最適化された、極めて健全な優良事業モデルである。視聴者の信頼が継続的に蓄積されており、M-1においても準々決勝・準決勝と堅実に駒を進める「負けない戦い」ができる実力を持つ。さらなる飛躍、すなわち決勝の舞台を掴み取るためには、既存の枠組みをあえて破壊するような「予測不能な裏切り」のスパイスが必要となる。` 
    };
    if (cvr >= 0.4) return { 
      icon: '🌱', rank: '損益分岐点', sub: 'Break-Even Point / M-1・2回戦・3回戦レベル', label: 'ミドルヒット領域（10万再生超）', 
      text: `標準的なエンゲージメント水準を確保しており、基礎的な技術力と構成力は市場に証明されている。しかし、数多いる実力派漫才師の中で「なぜ彼らでなければならないのか」という絶対的な理由付けに欠けている。現状の損益分岐点から抜け出すためには、キャラクターの深掘りや、他組には真似できない唯一無二のストロングポイントの確立にリソースを集中投下すべきである。` 
    };
    return { 
      icon: '📉', rank: '不良債権', sub: 'Non-Performing Loan / M-1・1回戦敗退レベル', label: 'ミドルヒット領域（10万再生超）', 
      text: `一定の認知は得られているものの、視聴者の心を揺さぶる決定打が不足している。「とりあえず見られる」という状態から、「思わず評価ボタンを押したくなる」という能動的なフェーズへの移行デザインが欠落している。ターゲットが誰なのかが曖昧になっている可能性が高く、ペルソナの再設定と、ネタの冒頭15秒における強烈なフックの再開発が必須である。` 
    };
  }

  // アーリーステージ（10万再生未満）
  if (cvr >= 3.0) return { 
    icon: '👑', rank: 'ユニコーン企業', sub: 'Unicorn / M-1決勝レベル', label: 'アーリーステージ（10万再生未満）', 
    text: `分母となる再生数こそ発展途上（アーリーステージ）であるが、一度触れた視聴者を確実に虜にするCVR ${cvr}%という数字は、もはや「暴力的なまでの引力」である。マス向けの最適化を一切無視した、純度の高いお笑いのコアがそこに存在する。この熱狂の種火を消すことなく、戦略的なSNS露出やライブでの実績構築を掛け合わせることで、瞬く間にスターダムへと駆け上がるSSR級の原石である。` 
  };
  if (cvr >= 1.5) return { 
    icon: '💎', rank: '優良事業', sub: 'Cash Cow / M-1・準々決勝～準決勝レベル', label: 'アーリーステージ（10万再生未満）', 
    text: `初期段階のプロジェクトとしては、申し分のないエンゲージメント率を記録している。ニッチな層に深く突き刺さる「独自の世界観（コンセプチュアルな強み）」が正しく市場に評価され始めている段階だ。この確固たる支持基盤をキャッシュカウ（資金源）として足場を固めつつ、少しずつ一般層にも伝わる翻訳（チューニング）を施していくことが、次なる成長への最短ルートとなる。` 
  };
  if (cvr >= 0.8) return { 
    icon: '🌱', rank: '損益分岐点', sub: 'Break-Even Point / M-1・2回戦・3回戦レベル', label: 'アーリーステージ（10万再生未満）', 
    text: `初期段階において標準以上のCVRを獲得しており、最低限の品質保証（クオリティ・アシュアランス）はクリアしている。しかし、爆発的なバズを生み出すための「起爆剤」がまだ投下されていない。漫才の後半に向けてのボルテージの上げ方や、記憶にこびりつく強烈なパンチラインの欠如が課題であり、構成のクライマックスにおけるカタルシスの最大化を図るべきである。` 
  };
  return { 
    icon: '📉', rank: '不良債権', sub: 'Non-Performing Loan / M-1・1回戦敗退レベル', label: 'アーリーステージ（10万再生未満）', 
    text: `残念ながら、現時点では市場からの能動的な評価を全く得られておらず、コンセプトレベルでの深刻なエラーが起きている。現状のまま回数を重ねてもサンクコスト（埋没費用）が膨らむばかりである。客観的なフィードバックを徹底的に収集し、「自分たちがやりたいお笑い」と「市場が求めているお笑い」の乖離を冷静に分析・すり合わせるという、痛みを伴う改革が必要不可欠だ。` 
  };
};
